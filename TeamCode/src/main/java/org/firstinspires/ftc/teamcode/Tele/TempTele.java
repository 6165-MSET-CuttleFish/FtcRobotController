...
public class TempTele extends OpMode {
    ...
    TriggerReader shootButton, powerShots;
    KeyReader[] readers;
    ...
    private void initializeButtons() {
        ...
        shootButton = new TriggerReader(g2, GamepadKeys.Trigger.RIGHT_TRIGGER);
        ...
        readers = new KeyReader[]{turretButton, incrementOffset, decrementOffset, shootButton, clawButton, shieldButton, wobbleButton, reverseMode, magButton, shooterMode, powerShots};
    }
    ...
    public void safety() {
        if (robotPose.getX() > 2) {
            ...
            if (shooter.getState() == Shooter.State.CONTINUOUS && turret.isIdle() && Magazine.currentRings != 0 && magazine.getState() == Magazine.State.DOWN
                    && shooter.isWithinTolerance() && shootButton.isDown()) {
                gunner.shoot();
            }
            turret.setState(Turret.State.TARGET_LOCK);
        }
