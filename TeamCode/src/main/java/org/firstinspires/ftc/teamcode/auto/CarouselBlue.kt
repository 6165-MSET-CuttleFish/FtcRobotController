    ...
    override fun runOpMode() {
        ...
        val trajectoryBuilder =
            robot.trajectorySequenceBuilder(startingPosition())
                ...
                .splineTo(Vector2d(-54.0, -37.0).flip(blue), Math.toRadians(-270.0).flip(blue))
                .setConstraints(getVelocityConstraint(20.0, Math.PI / 10, DriveConstants.TRACK_WIDTH), getAccelerationConstraint(20.0))
                .turn(Math.toRadians(-90.0).flip(blue))
                .back(19.0)
                .resetConstraints()
                .capstonePickup(capstone)
                .liftUp(deposit, getLevel(Details.location))
                .waitWhile(capstone::isDoingWork) // capstone loaded
                .splineTo(Vector2d(-24.5, -30.0).flip(blue), Math.toRadians(30.0).flip(blue))
                .setReversed(false)
                .dump(deposit)
